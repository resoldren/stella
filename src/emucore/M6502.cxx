...
bool M6502::execute(uInt32 number)
{
  ...
  bool ans;
#ifdef TIME_M6502_EXTRA
  std::map<Int32, uInt64> times;
  uInt64 maxTime = 0;
  Int32 maxIR = 0;
  uInt64 t1 = 0;
#endif
  ...
  for(;;)
  {
    for(; !myExecutionStatus && (number != 0); --number)
    {
      ...
      Int32 key = IR;
      std::chrono::high_resolution_clock::time_point timeA, timeB, timeC, timeD;
#endif
      switch(IR)
      {
      ...
      it->second += std::chrono::duration_cast<std::chrono::microseconds>(time2 - time1).count();
...
#ifdef TIME_M6502_EXTRA
         " maxIR: %d maxIRTime: %" PRIu64 "  %" PRIu64
#endif
         "\n",
...
#ifdef TIME_M6502_EXTRA2
  printf("here: %d\n", times.size());
  std::vector<std::pair<Int32, uInt64>> top(10);
  std::partial_sort_copy(times.begin(),
                         times.end(),
                         top.begin(),
                         top.end(),
                         [](std::pair<Int32, uInt64> const& l,
                            std::pair<Int32, uInt64> const& r)
                         {
